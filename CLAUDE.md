...
Four campaigns: **C1 Publish or Perish** (classic data-torture QRPs), **C2 The Methods
...
arsenal is present but loses because it raises suspicion). Plus free-play **Sandbox Lab** and
**Meta-Science Lab** (p-curve / funnel / power / equivalence).
...
- **`levels.js`** / **`levels.c2.js`** / **`levels.c3.js`** / **`levels.c4.js`** (`PSPSS_levels`) —
  `build(seed)` returns deterministic data + a pinned tuned `seed`; C2/C3/C4 levels carry
  `evaluate(state, ctx)`. C4 levels set `objective:'honest'` and return `decisionWin` (the goal is a
  defensible conclusion, not crossing the metric).
