...
(function () {
  ...
  // Chained after the first-visit intro video (including when the video is
  // skipped or errors — onboarding must not die with the asset), replayable
  // via the start screen's 👋 Welcome button. It introduces the PREMISE, not
  // answers: Hal stays a temptation mechanic, so the no-hints invariant holds.
  function showWelcome() {
    const body = el('div', { class: 'welcome-wrap' });
    const head = el('div', { class: 'welcome-head' });
    if (CHAR) head.appendChild(el('img', { class: 'welcome-img', src: CHAR.IMG, alt: CHAR.NAME }));
    head.appendChild(el('div', {}, [
      el('h2', { text: '👋 Welcome to the lab' }),
      el('div', { class: 'welcome-name', text: CHAR ? CHAR.NAME + ' — ' + CHAR.ROLE : 'Your new department' }),
    ]));
    body.appendChild(head);
    body.appendChild(el('div', { class: 'welcome-speech', html:
      '“Ah — the new hire. Sit. I\'m Hal. Emeritus. I <i>consult</i>.<br><br>' +
      'Here\'s the situation. Every dataset in this building has been rigged by the gods of tenure so the ' +
      'honest analysis comes up empty. Your job is to publish anyway: <b>diagnose</b> the flaw from the ' +
      'figures — diagnostics cost nothing — then apply exactly the right <b>Questionable Research ' +
      'Practice</b> until <b>p &lt; .05</b> falls out. Fewer moves, more stars. The statistics underneath ' +
      'are real, which I personally consider a design flaw.<br><br>' +
      'After every “publication” the <b>Debrief</b> reveals what the data actually held and what your ' +
      'manoeuvre truly did to the false-positive rate — with citations, the ghouls. The campaigns ' +
      'escalate: first you torture data, then whole models, then Bayes factors… and eventually the Open ' +
      'Science crowd makes you win <i>honestly</i>. Dark times.<br><br>' +
      'If you\'re ever stuck, click my face. I always know what to do.”' }));
    body.appendChild(el('div', { class: 'hint', text: '(He does not. It\'s log transforms. It\'s always log transforms.)' }));
    body.appendChild(el('div', { class: 'btnrow' }, [
      el('button', { class: 'btn', onclick: closeModal }, ['Begin — p < .05 awaits']),
      el('button', { class: 'btn ghost', onclick: showHow }, ['How it all works']),
    ]));
    modal('Welcome to PSPSS', body, { dismissable: true });
  }
  ...
  function showIntro(firstVisit) {
    ...
    const done = () => {
      ...
      try { v.pause(); } catch (e) {}
      back.remove();
      if (firstVisit === true) showWelcome(); // meet Hal once the credits roll
    };
